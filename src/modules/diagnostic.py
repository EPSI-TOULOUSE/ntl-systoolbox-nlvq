...
def diagnostiquer_windows(config: dict) -> dict:
    ...
    try:
        ...
        _afficher_section(f'Diagnostic Windows — {config["windows"]["ip"]}')
        ...
        for service in services_cibles:
            commande = f"(Get-Service -Name '{service}').Status"
        ...
        # --- Disque C: ---
        cmd_disque = (
            '$d = Get-PSDrive C;[math]::Round(($d.Used / ($d.Used + $d.Free)) * 100, 2)'
        )
...
def diagnostiquer_linux(config: dict) -> dict:
    ...
    try:
        ...
        _afficher_section(f'Diagnostic Linux — {config["linux"]["ip"]}')
